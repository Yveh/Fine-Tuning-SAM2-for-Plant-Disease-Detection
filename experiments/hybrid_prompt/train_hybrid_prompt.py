"""
Hybrid Prompt Training Script - Combines baseline (points) with box prompts
Uses both point prompts and bounding box prompts together for enhanced training
"""
import os
import random
import pandas as pd
...
def get_bounding_box(mask):
    ...
    Returns: [x_min, y_min, x_max, y_max] format (xyxy)
    """
...
def read_batch(data, visualize_data=False):
    """
    Modified to return BOTH points and bounding boxes for hybrid prompting
    """
    ...
    # Get all coordinates inside the eroded mask for point prompts
    # For hybrid prompts: use ONE point that matches with ONE box
    coords = np.argwhere(eroded_mask > 0)
    if len(coords) > 0:
        # Select ONE random point instead of multiple points
        yx = np.array(coords[np.random.randint(len(coords))])
        points.append([yx[1], yx[0]])

    # Get bounding box for the combined binary mask
    ...
    # Format boxes as expected by SAM2: shape (num_boxes, 4)
    # Ensure points and boxes have the same batch size (1)
    ...
    return Img, binary_mask, points, boxes, len(inds)
...
def main():
    print("="*70)
    print("HYBRID PROMPT TRAINING - Using BOTH points AND boxes together")
    print("="*70)
    ...
    # Training parameters
    MAX_STEPS = 20000  # Maximum steps (safety limit)
    FINE_TUNED_MODEL_NAME = "hybrid_prompt_sam2"
    MODEL_SAVE_DIR = "../../models/hybrid_prompt"
    ...
    convergence_detector = ConvergenceDetector(
        ...
        check_window=500    # Check every 500 steps
    )

    mean_iou = 0
    training_log = []

    print(f"\nStarting hybrid prompt training...")
    print(f"Max steps: {MAX_STEPS}")
    print(f"Convergence patience: 2000 steps")
    print(f"Check window: 500 steps")
    print(f"Using: Point prompts + Bounding box prompts")
    print("="*70)

    for step in range(1, MAX_STEPS + 1):
        with torch.amp.autocast(device_type='cuda'):
            image, mask, input_point, input_box, num_masks = read_batch(train_data, visualize_data=False)
            if image is None or mask is None or num_masks == 0:
                continue

            # For hybrid prompts: one point + one box = batch size of 1
            input_label = np.ones((1, 1))
            if not isinstance(input_point, np.ndarray) or not isinstance(input_label, np.ndarray):
                continue

            if input_point.size == 0 or input_label.size == 0 or input_box.size == 0:
                continue

            predictor.set_image(image)

            # Prepare prompts with BOTH points and boxes
            mask_input, unnorm_coords, labels, unnorm_box = predictor._prep_prompts(
                input_point, input_label, box=input_box, mask_logits=None, normalize_coords=True
            )
            ...
            # Pass BOTH points and boxes to prompt encoder
    ...
    # Save training log
    log_file = "../../results/hybrid_prompt_training_log.json"
    ...
    print("="*70)
    print("HYBRID PROMPT TRAINING COMPLETED!")
    print("="*70)
