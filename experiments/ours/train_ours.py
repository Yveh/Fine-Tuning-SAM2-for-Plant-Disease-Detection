"""
Ours: Complete Training Script
Combines BOTH:
1. Hybrid prompts (points + boxes together)
2. Data augmentation (flips, rotations, brightness, etc.)
"""
...
def read_batch(data, use_augmentation=True):
    """
    Read batch with hybrid prompts (points + boxes) and optional data augmentation
    """
    ...
    if use_augmentation:
        Img, ann_map = apply_augmentation(Img, ann_map)
...
def main():
    print("="*70)
    print("OURS: HYBRID PROMPT + DATA AUGMENTATION TRAINING")
    print("="*70)
    ...
    FINE_TUNED_MODEL_NAME = "ours_sam2"
    MODEL_SAVE_DIR = "../../models/ours"
    ...
    print(f"\nStarting hybrid prompt + data augmentation training...")
    print(f"Max steps: {MAX_STEPS}")
    print(f"Convergence patience: 2000 steps")
    print(f"Check window: 500 steps")
    print(f"Using: Point prompts + Box prompts + Data Augmentation")
    print("="*70)

    for step in range(1, MAX_STEPS + 1):
        with torch.amp.autocast(device_type='cuda'):
            image, mask, input_point, input_box, num_masks = read_batch(train_data, use_augmentation=True)
    ...
    # Save training log
    log_file = "../../results/ours_training_log.json"
