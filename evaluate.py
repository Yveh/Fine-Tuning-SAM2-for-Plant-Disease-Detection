"""
Generic SAM2 Evaluation (function-based)

Call:
    metrics = evaluate_sam2_model(
        model_path="../../models/baseline/baseline_sam2_final_7500.pt",
...
This evaluation matches the baseline-style metric:
- sample points inside GT (eroded)
...
def get_points(mask: np.ndarray, num_points: int = 30) -> Optional[np.ndarray]:
    points = []
...
def evaluate_sam2_model(
    ...
    """
    Evaluate any SAM2 checkpoint on leaf-seg test split.
    ...
    for entry in tqdm(test_data, desc=f"Evaluating {os.path.basename(model_path)}"):
        try:
            ...
            points = get_points(gt_mask, num_points=num_points)
            if points is None:
                continue
            ...
            with torch.no_grad():
                ...
                masks, scores, _ = predictor.predict(
                    point_coords=points,
                    point_labels=np.ones([points.shape[0], 1], dtype=np.float32),
...
if __name__ == "__main__":
    ...
    metrics = evaluate_sam2_model(
        # model_path="../models/baseline/baseline_sam2_5000.pt",
        # model_path="../models/data_augmentation/augmented_sam2_40000.pt",
        # model_path="../models/only_freeze_output/only_freeze_output_sam2_final_20000.pt",
        # model_path="../models/geo_data_augmentation/geo_augmented_sam2_final_20000.pt",
        # model_path="../models/data_augmentation_freeze/augmented_sam2_freeze_final_20000.pt",
        # model_path="../models/fine_tuned_sam2_10000.pt",
        model_path="../models/mixed_points/mixed_points_sam2_20000.pt",
